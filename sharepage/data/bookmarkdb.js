//#!本文件由share.js自动产生于Thu May 07 2015 12:08:08 GMT+0800 (CST), 产生命令行为: node share.js gen bookmark CRUD ..
  ...
  _Module.find({ moduleName: data.moduleName } 
    ,function(err, items){
      if(share.isConflict(items, _id))
        return fn(new Error('与现有bookmark名称重复，不能修改.'));
...
exports.list = function(cond, sort, page, fn){
  ...
  _Module.find(cond, defaultProjection)
    ...
    .exec(function(err, docs){
      if(err) console.trace("查询bookmark错误:", err);
...
exports.count = function(cond, fn){
  _Module.find(cond)
    .count(function(err, count){
      if(err) console.trace("统计bookmark数据量错误::", err);
...
exports.importCSV = function(filename, fn, updater, opts) {
    ...
    imp.import(function(err, count, cellIds){
        if(err) {
            console.trace("导入bookmark出错", err);
            return fn(err)
        };
...
if(isme(__filename)){
  if(process.argv.length > 2 && isme(__filename)){
    ...
    console.log('bookmarkdb.js '+ testcmd.join('|'));
  }
}
