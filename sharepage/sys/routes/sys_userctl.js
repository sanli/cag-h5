 ...
 */
var sysuserdb = require('../data/sys_userdb.js')
    // base function
    , share = require('../../../sharepage.js')
    , sf = require('../../../sharepage.js')
    , inspect = require('util').inspect
    , ObjectID = require('mongodb').ObjectID
    , upload = require('./upload')
    , sysorgdb = require('../data/sys_orgdb.js')
    , sysroledb = require('../data/sys_roledb.js')
...
exports.bindurl=function(app){
    share.bindurl(app, '/sysuser.html', { outType : 'page'}, exports.page);
    share.bindurl(app, '/sysuser/create', exports.create);
    share.bindurl(app, '/sysuser/update', exports.update);
    share.bindurl(app, '/sysuser/list', exports.list);
    share.bindurl(app, '/sysuser/retrive', exports.retrive);
    share.bindurl(app, '/sysuser/delete', exports.delete);
    share.bindurl(app, '/sysuser/count', exports.count);
    share.bindurl(app, '/sysuser/import', exports.import);
    share.bindurl(app, '/sysuser/export', exports.export);
}


// GUI页面
exports.page = function(req, res){
    res.render('sys/sys_userpage.html', {
        conf : require('../../../config.js'),
        title: sf.getTitle("用户管理"),
        target : conf.target,
        stamp : conf.stamp,
...
exports.create = function(req, res){
    var arg = share.getParam("创建新sysuser对象:", req, res, [CRUD.data]);
    if(!arg.passed)
       return;

    sysuserdb.create(arg.data, function(err, newDoc){
        if(err) return share.rt(false, "创建新sysuser对象出错:" + err.message, res);
...
exports.update = function(req, res){
    var arg = share.getParam("更新sysuser对象", req, res, [CRUD._id, CRUD.data]);
    ...
    sysuserdb.update( arg._id , data , function(err, cnt){
        if(err) {
            console.log(err);
            return share.rt(false, "更新sysuser出错:" + err.message, res);
        }
...
exports.list = function(req, res){
    var arg = share.getParam("查询sysuser列表", req, res, [PAGE.page, PAGE.cond, PAGE.sort]);
    if(!arg.passed)
        return;
...
exports.count = function(req, res){
    var arg = share.getParam("sysuser count", req, res, [PAGE.cond]);
    if(!arg.passed)
        return;
...
exports.retrive = function(req, res){
    var arg = share.getParam("retrive sysuser", req, res, [CRUD._id]);
    if(!arg.passed) return;

    sysuserdb.findById(arg._id, function(err, doc){
        if(err) return share.rt(false, "查询sysuser出错:" + err.message, res);
...
exports.import = function(req, res){
    var arg = share.getParam("import sysuser file", req, res, [IMP.file]);
    if(!arg.passed)
        return;
    var file = upload.settings.uploadpath + '/' + arg.file ,
        type = arg.type;

    sysuserdb.importCSV(file, type, function(err, cnt){
        if(err) return share.rt(false, "导入sysuser文件出错:" + err.message, res);
...
exports.export = function(req, res){
    var arg = getParam("sysuser list", req, res, [PAGE.cond, PAGE.sort]);
    if(!arg.passed)
        return;

    searchCondExp(arg.cond);
    fillUserDept(arg.cond, req);
    sysuserdb.query(arg.cond, arg.sort, function(err, docs){
        if(err) return rt(false, err.message, res);
        
        var filename = "export_" + arg.type + "_" + new Date().getTime() + ".csv";
        var exporter = sysuserdb.createExporter();
