...
/**
 * sysroleHTTP入口模块, 需要在主文件中添加map
 * var sysrole = require('./routes/sysrole').bindurl(app);
 */
var sysroledb = require('../data/sysroledb.js')
    , sysmoduledb = require('../data/sysmoduledb.js')
...
exports.bindurl=function(app){
    share.bindurl(app, '/sysrole.html', { outType : 'page'}, exports.page);
    share.bindurl(app, '/sysrole/create', exports.create);
    share.bindurl(app, '/sysrole/update', exports.update);
    share.bindurl(app, '/sysrole/list', exports.list);
    share.bindurl(app, '/sysrole/retrive', exports.retrive);
    share.bindurl(app, '/sysrole/delete', exports.delete);
    share.bindurl(app, '/sysrole/count', exports.count);
    share.bindurl(app, '/sysrole/import', exports.import);
    share.bindurl(app, '/sysrole/export', exports.export);
    //TODO: 扩展API
    share.bindurl(app, '/sysrole/listall', exports.listall);
}


// GUI页面
exports.page = function(req, res){
    sysmoduledb.getAllModule(function(err, modules){
        if(err) return share.rt(false, "查询系统模块出错:" + err.message, res);

        res.render('sysrolepage.html', {
            conf : require('../config.js'),
            title: sf.getTitle("角色权限管理"),
...
exports.create = function(req, res){
    var arg = share.getParam("创建新sysrole对象:", req, res, [CRUD.data]);
    if(!arg.passed)
       return;

   console.log(arg.data);
    sysroledb.create(arg.data, function(err, newDoc){
        if(err) return share.rt(false, "创建新sysrole对象出错:" + err.message, res);
...
exports.update = function(req, res){
    var arg = share.getParam("更新sysrole对象", req, res, [CRUD._id, CRUD.data]);
    ...
    var data = arg.data;
    delete data._id;
    sysroledb.update( arg._id , data , function(err, cnt){
        if(err) {
            console.log(err);
            return share.rt(false, "更新sysrole出错:" + err.message, res);
        }
...
exports.list = function(req, res){
    var arg = share.getParam("查询sysrole列表", req, res, [PAGE.page, PAGE.cond, PAGE.sort]);
    if(!arg.passed)
        return;
    ...
    sysroledb.list(arg.cond, arg.sort, page, function(err, docs){
...
exports.count = function(req, res){
    var arg = share.getParam("sysrole count", req, res, [PAGE.cond]);
    if(!arg.passed)
        return;

    share.searchCondExp(arg.cond);
    share.fillUserDataRule(arg.cond, req);
    sysroledb.count(arg.cond, function(err, count){
        if(err) return share.rt(false, err.message, res);
...
exports.retrive = function(req, res){
    var arg = share.getParam("retrive sysrole", req, res, [CRUD._id]);
    if(!arg.passed) return;

    sysroledb.findById(arg._id, function(err, doc){
        if(err) return share.rt(false, "查询sysrole出错:" + err.message, res);
...
exports.delete = function(req, res){
    var arg = share.getParam("delete sysrole", req, res, [CRUD._id]);
    if(!arg.passed) return;

    sysroledb.delete(arg._id , function(err, doc){
        if(err) return share.rt(false, "删除sysrole出错:" + err.message, res);
...
exports.import = function(req, res){
    var arg = share.getParam("import sysrole file", req, res, [IMP.file]);
    if(!arg.passed)
        return;
    var file = upload.settings.uploadpath + '/' + arg.file ,
        type = arg.type;

    sysroledb.importCSV(file, type, function(err, cnt){
        if(err) return share.rt(false, "导入sysrole文件出错:" + err.message, res);
...
exports.export = function(req, res){
    var arg = getParam("sysrole list", req, res, [PAGE.cond, PAGE.sort]);
    if(!arg.passed)
        return;

    searchCondExp(arg.cond);
    fillUserDept(arg.cond, req);
    sysroledb.query(arg.cond, arg.sort, function(err, docs){
        if(err) return rt(false, err.message, res);
        
        var filename = "export_" + arg.type + "_" + new Date().getTime() + ".csv";
        var exporter = sysroledb.createExporter();
...
exports.listall = function(req, res){
    ...
    sysroledb.getAllRole(function(err, docs){
        if(err) return share.rt(false, err.message, res);
        share.rt(true, {docs: docs}, res);
    });
};
