 ...
 */
var sysmoduledb = require('../data/sysmoduledb.js')
    // base function
...
exports.bindurl=function(app){
    share.bindurl(app, '/sysmodule.html', { outType : 'page'}, exports.page);
    share.bindurl(app, '/sysmodule/create', exports.create);
    share.bindurl(app, '/sysmodule/update', exports.update);
    share.bindurl(app, '/sysmodule/list', exports.list);
    share.bindurl(app, '/sysmodule/retrive', exports.retrive);
    share.bindurl(app, '/sysmodule/delete', exports.delete);
    share.bindurl(app, '/sysmodule/count', exports.count);
    share.bindurl(app, '/sysmodule/import', exports.import);
    share.bindurl(app, '/sysmodule/export', exports.export);
}

// GUI页面
exports.page = function(req, res){
    res.render('sysmodulepage.html', {
        conf : require('../../../config.js'),
        title: sf.getTitle("系统模块管理"),
...
exports.create = function(req, res){
    var arg = share.getParam("创建新sysmodule对象:", req, res, [CRUD.data]);
    if(!arg.passed)
       return;

    sysmoduledb.create(arg.data, function(err, newDoc){
        if(err) return share.rt(false, "创建新对象出错:" + err.message, res);
...
exports.update = function(req, res){
    var arg = share.getParam("更新sysmodule对象", req, res, [CRUD._id, CRUD.data]);
    ...
    var data = arg.data;
    delete data._id;
    sysmoduledb.update( arg._id , data , function(err, cnt){
        if(err) {
            console.log(err);
            return share.rt(false, "更新sysmodule出错:" + err.message, res);
        }
...
exports.list = function(req, res){
    var arg = share.getParam("查询sysmodule列表", req, res, [PAGE.page, PAGE.cond, PAGE.sort]);
    if(!arg.passed)
        return;
    ...
    sysmoduledb.list(arg.cond, arg.sort, page, function(err, docs){
...
exports.count = function(req, res){
    var arg = share.getParam("sysmodule count", req, res, [PAGE.cond]);
    if(!arg.passed)
        return;

    share.searchCondExp(arg.cond);
    share.fillUserDataRule(arg.cond, req);
    sysmoduledb.count(arg.cond, function(err, count){
        if(err) return share.rt(false, err.message, res);
...
exports.retrive = function(req, res){
    var arg = share.getParam("retrive sysmodule", req, res, [CRUD._id]);
    if(!arg.passed) return;

    sysmoduledb.findById(arg._id, function(err, doc){
...
exports.delete = function(req, res){
    var arg = share.getParam("delete sysmodule", req, res, [CRUD._id]);
    if(!arg.passed) return;

    sysmoduledb.delete(arg._id , function(err, doc){
...
exports.import = function(req, res){
    var arg = share.getParam("import sysmodule file", req, res, [IMP.file]);
    if(!arg.passed)
        return;
    var file = upload.settings.uploadpath + '/' + arg.file ,
        type = arg.type;

    sysmoduledb.importCSV(file, type, function(err, cnt){
        if(err) return share.rt(false, "导入文件出错:" + err.message, res);
...
exports.export = function(req, res){
    var arg = share.getParam("sysmodule list", req, res, [PAGE.cond, PAGE.sort]);
    if(!arg.passed)
        return;

    share.searchCondExp(arg.cond);
    share.fillUserDataRule(arg.cond, req);
    sysmoduledb.query(arg.cond, arg.sort, function(err, docs){
        if(err) return share.rt(false, err.message, res);
        
        var filename = "export_" + arg.type + "_" + new Date().getTime() + ".csv";
        var exporter = sysmoduledb.createExporter();
        share.exportToCSVFile(docs, filename, exporter, function(err){
            if(err)
                return share.rt(false, "导出文件错误:" + err , res);

            return share.rt(true, {file: filename, fname: "SFMIS-LY-export.csv"}, res);
        });
    });
};
