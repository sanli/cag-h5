...
var Module = $.extend(new $M(), {
    ...
    loadPageData: function(stateCond, page){
        ...
        // 列出整个树
        $('#treeView').spin;
        PG.loadOrg(function(orgs){
            var $resultTarget = $('#treeView');
            $M.fillResult($resultTarget, {
                tree : orgs.tree,
                orgs : orgs,
                tempId : 'treeViewTmpl',
    ...
    bind : function(){
        ...
        // 数据改变，自动重新计算
        $('#module-form').on('change','', function(){
            $('#orgFullName').val($('#parentFullName').val() + $('#orgName').val());
        });
    },
    ...
    createModule: function(){
        function _createModule(condition, fn, fail){
            $M.doupdate('/sysorg/create', condition, { successfn : fn , failfn: fail});
        };
    ...
    updateModule: function(_id, options){
        ...
        _updateModule = function(condition, fn, fail){
            $M.doupdate('/sysorg/update', condition, { successfn : fn , failfn: fail});
        },
        // 查询详细信息
        _loadDataDetail = function(_id, fn){
            $M.doquery('/sysorg/retrive', {_id : _id}
                , { successfn : fn ,
