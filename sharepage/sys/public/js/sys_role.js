...
var Module = $.extend(new $M(), {
    ...
    loadPageData: function(stateCond, page){
        ...
        // 处理需要做正则表达式查询的条件
        if(cond.roleName)
            cond.roleName = 'Reg(' + cond.roleName + ')';
    ...
    createModule: function(){
        function _createModule(condition, fn, fail){
            $M.doupdate('/sysrole/create', condition, { successfn : fn , failfn: fail});
        };
        ...
        $.showmodal('#moduleDlg', function(){
            if ($('#module-form').validate().form()){
                ...
                Module.extractModuleRight(data);
    ...
    updateModule: function(_id, options){
        ...
        _updateModule = function(condition, fn, fail){
            $M.doupdate('/sysrole/update', condition, { successfn : fn , failfn: fail});
        },
        // 查询详细信息
        _loadDataDetail = function(_id, fn){
            $M.doquery('/sysrole/retrive', {_id : _id}
                , { successfn : fn ,
    ...
    // ========== 请尽量在这一行后面加入扩展代码 ==========
    // 从form中提取权限列表
    extractModuleRight : function(data){
        var rightsMap = {}, rights= [];
        $("input[name=rights]").each(function(idx, input){
            var $input = $(input),
                modulename = $input.data('modulename'),
                right = $input.data('right');
            if(!$input.prop('checked'))
                return ;

            if( rightsMap[modulename] ){
                rightsMap[modulename].push(right);
            }else{
                rightsMap[modulename] = [right];
            }
        });
        // for( moduleName in rightsMap){
        //     rights.push({ moduleName : moduleName , rules : rightsMap[moduleName]});
        // }
        //data.rights = rights;
        data.rights = rightsMap;
    },
    // 设置权限列表
    fillModuleRight : function(data){
        if(data.rights){
            $.each(data.rights, function(moduleName, rights){
                $.each(rights, function(idx, right){
                    $('#module-form input[data-modulename=' + moduleName + '][data-right=' + right + ']').prop('checked', true);
                });
            });    
        }
    }
