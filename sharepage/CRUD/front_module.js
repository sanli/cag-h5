...
var Module = $.extend(new $M(), {
    ...
    // 事件处理函数
    //====================================================================================================================
    //需要新建一个<M%=module_name %M>
    createModule: function(){
        function _createModule(condition, fn, fail){
            $M.doupdate('/<M%=module_name %M>/create', condition, { successfn : fn , failfn: fail});
        };
    ...
    // 编辑<M%=module_name %M>信息
    updateModule: function(_id, options){
        // 更新楼宇信息
        _updateModule = function(condition, fn, fail){
            $M.doupdate('/<M%=module_name %M>/update', condition, { successfn : fn , failfn: fail});
        },
        // 查询详细信息
        _loadDataDetail = function(_id, fn){
            $M.doquery('/<M%=module_name %M>/retrive', {_id : _id}
                , { successfn : fn ,
    ...
    onExport : function(e){
        ...
        $e.spin();
        $M.doquery('/<M%=module_name %M>/export'
