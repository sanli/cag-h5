...
var Module = $.extend(new $M(), {
        ...
        Module.listPage(type, cond, sortarg, page
            , function(module){
                ...
                $M.fillResult($resultTarget, {
                    ...
                    bodyId: <M%=module_name %M> + 'cellTbody',
                    tempId : <M%=module_name %M> + 'CellTable',
                    sort: sort,
                });
    ...
    updateModule: function(_id, options){
        ...
        var queryBuildingSFData = function(_id){
            $('#<M%=module_name %M>SFDataTB').spin();
            $M.doquery('/sfdata'
                , { _id : _id, type : '<M%=module_name %M>' }
                , { 
        ...
        updateModule = function(condition, fn, fail){
            $M.doupdate('/<M%=module_name %M>/u', condition, { successfn : fn , failfn: fail});
        },
        // 查询楼宇详细信息
