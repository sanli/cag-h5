...
(function($) {
    ...
    $.ScalebarSizeAndTextRenderer = {
        ...
        METRIC_LENGTH: function(ppm, minSize) {
            return getScalebarSizeAndTextForMetric(ppm, minSize, "米");
        },
        /**
        ...
        STANDARD_TIME: function(pps, minSize) {
            var maxSize = minSize * 2;
            if (maxSize < pps * 60) {
                return getScalebarSizeAndTextForMetric(pps, minSize, "秒", false);
            }
            var ppminutes = pps * 60;
            if (maxSize < ppminutes * 60) {
                return getScalebarSizeAndText(ppminutes, minSize, "分钟", true);
            }
            var pph = ppminutes * 60;
            if (maxSize < pph * 24) {
                return getScalebarSizeAndText(pph, minSize, "小时", true);
            }
            var ppd = pph * 24;
            if (maxSize < ppd * 365.25) {
                return getScalebarSizeAndText(ppd, minSize, "天", true);
            }
            var ppy = ppd * 365.25;
            return getScalebarSizeAndText(ppy, minSize, "年", true);
        },
        /**
    ...
    function getScalebarSizeAndText(ppm, minSize, unitSuffix, handlePlural) {
        ...
        var plural = handlePlural && factor > 1 ? "秒" : "";
        return {
            size: size,
    ...
    function getWithUnit(value, unitSuffix) {
        if (value < 0.000001) {
            return value * 1000000000 + " 纳" + unitSuffix;
        }
        if (value < 0.001) {
            return value * 1000000 + " 微" + unitSuffix;
        }
        if (value < 0.01) {
            return value * 1000 + " 毫" + unitSuffix;
        }
        if (value < 1) {
            return value * 100 + " 厘" + unitSuffix;
        }
        if (value >= 1000) {
            return value / 1000 + " 千" + unitSuffix;
        }
