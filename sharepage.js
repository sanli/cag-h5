/**
 * sharepage.js 一些NodeJS端常用的函数和对象
 */
...
// @type : api / page
// @opt : 验证方法和重定向URL
function _createRestrictFn( type, opt ){
	opt = opt || {
		// 用户session信息存放的位置
		userSessionKey : 'user',
		// 没有登录需要重定向到的URL
	...
	if(type === 'api'){
		return function apiRestrict(req, res, next){
			if (req.session[opt.userSessionKey]) {
				next();
			} else {
...
var checkfuntions = {
	'tourist_page' : _createRestrictFn('page', { 
			userSessionKey : 'tourist',
			redirectUrl : '/main.html'
		}),
	'tourist_api' : _createRestrictFn('api', { 
			userSessionKey : 'tourist',
			redirectUrl : '/main.html'
		}),	
	'user_page' : _createRestrictFn('page', { 
			userSessionKey : 'user',
			redirectUrl : '/signin.html'
		}),
	'user_api' : _createRestrictFn('api', { 
			userSessionKey : 'user',
			redirectUrl : '/signin.html'
		}),
}
...
function bindurl(app, url, opt, fn){
    ...
    opt = extend({  method : 'all',
                    needAuth : true,
                    authRule : 'user',  // tourist
                    outType : 'api' } , opt);

    // 如果需要重定向，则所有的请求都定向到新站点，用于整体迁移服务
    if(conf.redirect){
  	...
  	if(opt.needAuth){
	  	var restrictfn = checkfuntions[ opt.authRule + '_' + opt.outType ];
	  	if(!restrictfn)
	  		throw new Error('无效的验证参数:' + opt.authRule + '_' + opt.outType);
...
var getreq = function(msg, req, res, params){
	...
	var fun = function(param){
		...
		arg[param.name] = ( req.param(param.key) !== undefined 
			? req.param(param.key) 
			: typeof (param.default) != 'object' 
				? param.default : extend({}, param.default) );
	};
...
exports.isme = function(filename){
...
exports._ResultByState = exports.resultByState = resultByState; // =============== 输出结果到Page ============
...
exports._assertNotNull = exports.assertNotNull = assertNotNull; // =============== Assert ============
 ...
 * 处理查询条件
 */
exports.processSearchCond = function(cond, opt){
	//处理正则化条件
	if(opt.regexp){
		var fields = opt.regexp;
		fields.forEach(function(field){
			if(cond[field] && typeof cond[field] === 'string')
				cond[field] = new RegExp(cond[field]);
		})
	}
}

// 查询表达式处理,根据查询条件中的表达式，创建MongoDB的查询条件
// 表达式都是字符串,形如：
//   Eq(等于这个值)   --> 需要等于这个值
//   Gt : 大于指定值
//   Lt : 小于指定值
//   GtLt(下限值,上限值)  --> 大于下限，小于上限
//   Reg(字符串)  --> 正则表达式匹配的内容
//   简单字符串    --> 需要等于这个值  
function searchCondExp(cond, opt){
	function processCond(cond, condName, value){
		var values = value.replace(/(\(|,|\))/g, '#').split('#');
	        checkType = values[0];

	    switch (checkType){
	    	case 'GtLt' : 
		    	var min = parseInt(values[1]), max = parseInt(values[2]);
		        //{ $and: [ { price: 1.99 }, { qty: { $lt: 20 } }, { sale: true } ]
		        cond[condName] = { $gt : min , $lt : max };  
		        break;

		    case 'Lt' :
		    	var max = parseInt(values[1]);
		    	cond[condName] = { $lt : max };
		        break;

		    case 'Gt' :
		    	var min = parseInt(values[1]);
	    		cond[condName] = { $gt : min }; 
	    		break;
	    	...
	    	case 'Reg' :
	    		cond[condName] = new RegExp(values[1]);
	    		break;
    ...
    for( condName in cond){
    	if( typeof cond[condName] === 'string'){
    		processCond(cond, condName, cond[condName]);	
    	}else if(Array.isArray(cond[condName])){
	    	//{ $or : [ { name: /中国移动安徽有限公司安庆/} , { buildingId: /中国移动安徽有限公司安庆/ }]
	    	console.log("cond[condName]:%s" , inspect(cond[condName]));
	    	cond[condName].forEach(function(orCond){
	    		console.log("console.log(orCond):", orCond);
	    		searchCondExp(orCond);
	    		console.log("console.log(orCond):", orCond);
	    	});
	    	console.log("cond[condName]:%s" , inspect(cond[condName]));
    	}
...
exports.searchCondExp = searchCondExp;

// ====================== 检查是否已经存在重复对象 ============
// 检查items中除了当前对象，是否还包含其他对象，如果包含，调用fn
exports.isConflict = function(items, _id){
	if(items.length > 0){
        var msg = [];
...
exports.pinyin = function(){

	// 在对象中搜索
	function arraySearch(l1, convert){
	    for (var name in PinYin){
	        if (PinYin[name].indexOf(l1)!=-1) {
	            return convert(name); break;
	        }
	    }
	    return false;
	};
	...
	function _searchPingyin(l1, fn){
		var l2 = l1.length;
	    var I1 = "";
	    var reg = new RegExp('[a-zA-Z0-9\- ]');
	    for (var i=0;i<l2;i++) {
	        var val = l1.substr(i,1);
	        var name = arraySearch(val,fn);
	        if(reg.test(val)) {
	            I1 += val;
	        } else if (name!==false) {
	            I1 += name;
	...
	function ConvertPinyin(l1) {
	    return _searchPingyin(l1, ucfirst);
	};

	function ConvertPinyinToFirst(l1){
		return _searchPingyin(l1, getfirst);
	};
	
	return {
		full : ConvertPinyin,
		first : ConvertPinyinToFirst,
	};
}; // 把汉字转化成拼音

// ============== 日期常用函数 ================

// 返回指定日期的月编号，月编号是从公元元年到当前经过的月份
// 编号从0开始。如果没有传入日期，就取当前日期
exports.getMonthIndex = function(date){
	var date = date || new Date();
	return date.getFullYear() * 12 + date.getMonth();
}

// ============== 对象操作常用函数  ================
// 返回一个对象，递归清除其中的指定值，与underscore.omit()功能相似
function deepOmit(obj, keys) {
	var copy = {};
	for (var key in obj) {
	  if (!us.contains(keys, key)){
	  	if(typeof(obj[key]) !== 'object'){
	  		copy[key] = obj[key];	
	  	}else{
	  		copy[key] = deepOmit(obj[key], keys);
	  	}
	  } 
	}
	return copy;
...
// ============== Cache管理函数  ================
// 创建一个查询全局缓冲数据的函数，自动对查询出来的数据作缓冲处理
var cache = require('memory-cache');
function _CreateCachedGetAllFn(cachekey, fnname, _Module, query, sort){
	query = query || {};
	sort = sort || {};
	return {
		// 查询函数
		list : function(fn){
			...
			_Module.find(query).sort(sort)
				.exec(function(err, docs){
				    if(err){
				      console.trace(fnname + " error:", err);
				    }

				    var objs = docs.map(function(doc){ return doc.toObject() });
				    cache.put(cachekey, objs);
				    fn(err, objs);
				});
		},
		// 清理Cache函数
...
exports._CreateCachedGetAllFn = _CreateCachedGetAllFn;
