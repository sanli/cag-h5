//#!本文件由share.js自动产生于Mon Jan 26 2015 15:48:06 GMT+0800 (CST), 产生命令行为: node share.js gen message CRUD ..
/**
 * messageHTTP入口模块, 需要在主文件中添加map
 * var message = require('./routes/message');
 * app.all('/message/list', apiRestrict, message.list);
 * app.all('/message/count', apiRestrict, message.count);
 * app.all('/message/import', apiRestrict, message.import);
 * app.all('/message/retrive', apiRestrict, building.retrive);
 * app.all('/message/update', apiRestrict, gisobj.updateBuilding);
 * app.all('/message/delete', apiRestrict, building.delete);
 * app.all('/message/export', apiRestrict, building.export);
 */
var messagedb = require('../data/messagedb.js')
...
exports.bindurl=function(){
    bindurl('/message.html', { outType : 'page'}, exports.page);
    bindurl('/message/list', exports.list);
    bindurl('/message/retrive', exports.retrive);
    bindurl('/message/update', exports.update);
    bindurl('/message/delete', exports.delete);
    bindurl('/message/count', exports.count);
    bindurl('/message/import', exports.import);
    bindurl('/message/export', exports.export);
}

// 楼宇页面
exports.page = function(req, res){
    res.render('messagepage.html', {
        title: "画作管理"
    });
};
...
exports.list = function(req, res){
    var arg = getParam("message list", req, res, [PAGE.page, PAGE.cond, PAGE.sort, PAGE.type]);
    if(!arg.passed)
        return;
    ...
    messagedb.list(arg.type, arg.cond, page, arg.sort, function(err, docs){
...
exports.count = function(req, res){
    var arg = getParam("message count", req, res, [PAGE.cond, PAGE.type]);
    if(!arg.passed)
        return;

    searchCondExp(arg.cond);
    fillUserDept(arg.cond, req);
    messagedb.count(arg.type, arg.cond, function(err, count){
        if(err) return rt(false, err.message, res);
...
exports.retrive = function(req, res){
    var arg = getParam("retrive message", req, res, [CRUD._id]);
    if(!arg.passed) return;

    messagedb.findById(arg._id, function(err, doc){
...
exports.delete = function(req, res){
    var arg = getParam("delete message", req, res, [CRUD._id]);
    if(!arg.passed) return;

    messagedb.delete(arg._id , _ResultByState(res, function(err, doc){
...
exports.import = function(req, res){
    var arg = getParam("import message file", req, res, [IMP.file, PAGE.type]);
    if(!arg.passed)
        return;
    var file = upload.settings.uploadpath + '/' + arg.file ,
        type = arg.type;

    messagedb.importCSV(file, type, function(err, cnt){
...
exports.export = function(req, res){
    var arg = getParam("message list", req, res, [PAGE.cond, PAGE.sort, PAGE.type]);
    if(!arg.passed)
        return;

    searchCondExp(arg.cond);
    fillUserDept(arg.cond, req);
    messagedb.query(arg.type, arg.cond, arg.sort, function(err, docs){
        if(err) return rt(false, err.message, res);
