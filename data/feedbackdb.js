//#!本文件由share.js自动产生于Mon Jun 01 2015 12:02:31 GMT+0800 (CST), 产生命令行为: node share.js gen feedback CRUD ..
...
exports.create = function(data, fn){
  ...
  _Module.find({ moduleName: data.moduleName}
    , function(err, items){
      ...
      if(items.length > 0)
        return fn(new Error('与现有用户反馈名称重复，不能创建'));

      var module = new _Module(data);
      _Module.create(module, function(err, newModule){
        if(err) console.log("新建feedback出错 :" + err);
        fn(err, newModule);
      });
    });
};
  ...
  _Module.find({ moduleName: data.moduleName } 
    ,function(err, items){
      if(share.isConflict(items, _id))
        return fn(new Error('与现有feedback名称重复，不能修改.'));
...
exports.list = function(cond, sort, page, fn){
  ...
  _Module.find(cond, defaultProjection)
    ...
    .exec(function(err, docs){
      if(err) console.trace("查询feedback错误:", err);
...
exports.count = function(cond, fn){
  _Module.find(cond)
    .count(function(err, count){
      if(err) console.trace("统计feedback数据量错误::", err);
...
exports.importCSV = function(filename, fn, updater, opts) {
    ...
    imp.import(function(err, count, cellIds){
        if(err) {
            console.trace("导入feedback出错", err);
            return fn(err)
        };
...
if(isme(__filename)){
  if(process.argv.length > 2 && isme(__filename)){
    ...
    console.log('feedbackdb.js '+ testcmd.join('|'));
  }
}
