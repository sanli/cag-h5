...
// 创建索引 
// TODO: 根据实际结构确定索引结构
footprints.index({_id : 1})
    .index({moduleName : 1});
var footprints = mongoose.model('footprints', footprints ),
    _Module = footprints ;
...
exports.create = function(data, fn){
  ...
  _Module.find({ moduleName: data.moduleName}
    , function(err, items){
      ...
      if(items.length > 0)
        return fn(new Error('与现有足迹名称重复，不能创建'));

      var module = new _Module(data);
      _Module.create(module, function(err, newModule){
        if(err) console.log("新建footprints出错 :" + err);
        fn(err, newModule);
      });
    });
};
  ...
  _Module.find({ moduleName: data.moduleName } 
    ,function(err, items){
      if(share.isConflict(items, _id))
        return fn(new Error('与现有footprints名称重复，不能修改.'));
...
exports.list = function(cond, sort, page, fn){
  ...
  _Module.find(cond, defaultProjection)
    ...
    .exec(function(err, docs){
      if(err) console.trace("查询footprints错误:", err);
...
exports.count = function(cond, fn){
  _Module.find(cond)
    .count(function(err, count){
      if(err) console.trace("统计footprints数据量错误::", err);
...
exports.importCSV = function(filename, fn, updater, opts) {
    ...
    imp.import(function(err, count, cellIds){
        if(err) {
            console.trace("导入footprints出错", err);
            return fn(err)
        };
...
if(isme(__filename)){
  if(process.argv.length > 2 && isme(__filename)){
    ...
    console.log('footprintsdb.js '+ testcmd.join('|'));
  }
}
