...
artical.index({_id : 1})
    .index({moduleName : 1});
var artical = mongoose.model('artical', artical ),
    _Module = artical ;
  ...
  _Module.find({ moduleName: data.moduleName } 
    ,function(err, items){
      if(share.isConflict(items, _id))
        return fn(new Error('与现有artical名称重复，不能修改.'));
...
exports.list = function(cond, sort, page, fn){
  ...
  _Module.find(cond, defaultProjection)
    ...
    .exec(function(err, docs){
      if(err) console.trace("查询artical错误:", err);
...
exports.count = function(cond, fn){
  _Module.find(cond)
    .count(function(err, count){
      if(err) console.trace("统计artical数据量错误::", err);
...
if(isme(__filename)){
  if(process.argv.length > 2 && isme(__filename)){
    ...
    console.log('articaldb.js '+ testcmd.join('|'));
  }
}
