//#!本文件由share.js自动产生于Mon Jan 26 2015 15:47:17 GMT+0800 (CST), 产生命令行为: node share.js gen messagelink CRUD ..
/**
 * messagelink数据库访问类
 */
//基站数据访问
...
exports.list = function(type, cond, sort, page, fn){
  ...
  Module.find(cond, defaultProjection)
    ...
    .exec(function(err, docs){
      if(err) console.trace("query messagelink page error:", err);
...
exports.query = function(type, cond, sort, fn){
    ...
    .sort(sort)
    .exec(function(err, docs){
      if(err) console.trace("query messagelink error:", err);
...
exports.count = function(type, cond, fn){
  Module.find(cond)
    .count(function(err, count){
      if(err) console.trace("query messagelink count error:", err);
...
exports.importCSV = function(filename, type, fn) {
    ...
    var importcsvfile = function(Module, fields, filename, fn){
        ...
        .on("end", function (count) {
            console.log("importcsvfile success messagelink filename:" + filename);
            fn(null, count - 1);
        })
...
if(isme(__filename)){
  if(process.argv.length > 2 && isme(__filename)){
    ...
    console.log('messagelinkdb.js '+ testcmd.join('|'));
  }
}
